...
def gen_pydyp_dyp(desc: GrammarDescription) -> List[str]:
    lines = PYDYP_DYP_PRELUDE.split('\n')
    lines.extend(PYDYP_DYP_REST.format(
        ...
        token_parsers='\n'.join(chain((f"token_{token}: {token}_ {{ tok \"{token}\" }}"
                                       for token in chain(desc.tokens.named,
                                                          desc.tokens.nameless)),
                                      (f"token_{token}: {token}_ {{ tok $1 }}" for token, _ in desc.tokens.typed))),
...
def str_of_production(production: Production, desc: GrammarDescription, rule_name: str, production_no: int = 0) -> str:
    ...
    for component in production.components:
        if isinstance(component, Terminal):
            parts.append('token_' + desc.terminal_names[component])
