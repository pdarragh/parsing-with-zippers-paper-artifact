...
let rec parse_tree (l : node) : ast list =
  ...
  let result = (if l.key == parse_tree_gensym
  then (l.ast)
        ...
        let result = match l.tag with
          ...
          | Eps_tag t ->
             ...
             t'
          | Red_tag f ->
             let z0 = parse_tree (List.hd l.children) in
             let z = f z0 in
             z
          | Alt_tag ->
             let z0 = List.map parse_tree l.children in
             let z = List.concat z0 in
             z
          | Seq_tag ->
             match l.children with
             | [] ->
  ...
  | (Seq_tag, _) -> let_result
     ...
     (fun result ->
       ...
       | x :: xs ->
          ...
          else let rec r (vs : node list) (xs : node list) : (node list * node * node list) list = match xs with
                 | [] -> []
                 | x :: xs ->
                    if not (nullable x)
