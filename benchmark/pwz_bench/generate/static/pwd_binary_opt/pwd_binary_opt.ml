...
let rec false_node = { tag = Unknown_tag;
                       child1 = false_node;
                       child2 = false_node;
                       nullable = Nullable_unvisited;
                       listeners = [];
                       key = false_token;
                       value = false_node;
                       ast = [] }

let make_node tag child1 child2 nullable =
  { tag = tag;
    child1 = child1;
    child2 = child2;
    nullable = nullable;
...
let make_opt_seq_node left right = match left.tag with
  ...
  | Seq_tag -> make_red_node (make_seq_node left.child1 (make_seq_node left.child2 right))
                 (fun ts -> List.map (fun t -> match t with
                                               Pyast.Ast (l1, [t1; Pyast.Ast (l2, [t2; t3])]) ->
                                                 Pyast.Ast (l2, [Pyast.Ast (l1, [t1; t2]); t3])) ts)
  | Eps_tag t -> make_red_node right
                   (fun ts2 -> List.concat (List.map (fun t1 -> List.map (fun t2 -> Pyast.Ast ("<seq>", [t1; t2])) ts2) (Lazy.force t)))
  | Red_tag f -> make_red_node (make_seq_node left.child1 right)
                   (fun ts -> List.concat (List.map (function Pyast.Ast (l, [t1; t2]) -> List.map (fun t' -> Pyast.Ast (l, [t'; t2])) (f [t1])) ts))
...
let alt_node_set_opt node left right = match left.tag with
  | Eps_tag content -> (match right.tag with
                        | Eps_tag content2 -> eps_node_set node (lazy (List.append (Lazy.force content) (Lazy.force content2))); true
...
let seq_node_set_opt_left node left right =
  ...
  | Seq_tag -> red_node_set node (make_opt_seq_node left.child1 (make_opt_seq_node left.child2 right))
                 (fun ts -> List.map (function Pyast.Ast (l1, [t1; Pyast.Ast (l2, [t2; t3])]) -> Pyast.Ast (l2, [Pyast.Ast (l1, [t1; t2]); t3])) ts);
               true
                ...
                (match l.child2.tag with
                 ...
                 | Eps_tag t ->
                   red_node_set l l.child1
                     (fun ts1 -> List.concat (List.map (fun t1 -> List.map (fun t2 -> Pyast.Ast ("<seq>", [t1; t2])) (Lazy.force t)) ts1));
                   true
                 | Red_tag f ->
                   red_node_set l (make_seq_node l.child1 l.child2.child1)
                     (fun ts -> List.concat (List.map (function Pyast.Ast (l, [t1; t2]) -> List.map (fun t' -> Pyast.Ast (l, [t1; t'])) [t2]) ts));
                   true
                 | _ -> false))
          | _ -> false in
        if reoptimize
