module PwdBinaryParserInterface = struct
  ...
  let parse (tokens : tok list) : res = Pwd_binary.parse tokens Pwd_binary_pygram.pwd_binary_rule_file_input

  let process_result (result : res) : Pyast.ast =
    match result with
    | [t] -> t
    | _ -> let header = Printf.sprintf "Invalid number of resulting AST trees in PwD-binary parse: %d." (List.length result) in
