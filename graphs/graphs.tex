...
\item This document reads graph data from files named \code{cubic.csv} and
      \code{all-bench-results.csv}, so those files should be created before
...
\pgfplotstableread{cubic.csv}\benchmarkData
\begin{tikzpicture}
\begin{axis}[
...
\noindent \centering{}\noindent \begin{center}
\pgfplotstableset{col sep=comma}
\pgfplotstableread{all-bench-results.csv}\benchmarkData
